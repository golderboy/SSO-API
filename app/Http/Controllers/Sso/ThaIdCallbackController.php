...
class ThaIdCallbackController extends Controller
{
    public function __invoke(
        ...
        ThaIdIdentityProvider $provider,
        ...
        try {
            ...
            $grants = $policy->eligible($user, $application);

            if ($grants->isEmpty()) {
                throw new BrokerRequestException(
                    'access_denied',
                    'No effective access grant exists for this application.',
                    403,
                );
            }
    ...
    private function validatedPayload(Request $request): array
    {
        ...
        if ($hasError) {
            ...
            if (
                ...
                throw new BrokerRequestException(
                    'invalid_request',
                    'The ThaID error callback is invalid.',
                );
            }
        ...
        if (
            ! is_string($code)
            || preg_match('/^[A-Za-z0-9._~-]{8,2048}$/D', $code) !== 1
        ) {
            throw new BrokerRequestException(
                'invalid_request',
                'The ThaID authorization code is invalid.',
            );
        }
